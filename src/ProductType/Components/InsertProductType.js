...
const InsertProductType = ({ Id, setChange, change }) => {
  ...
  async function GetProductTypeByIdR($id) {
    console.log($id);
    if (Id !== undefined && Id !== 0) {
      setUpd(true);
      const { data } = await GetByIdProductType($id);
      setObj((obj) => ({
        ...obj,
        Id: $id,
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      if (isEdit) {
        var { data } = await UpdateProductType(values);
        ...
        console.log(values);
        var { data } = await RegisterProductType(values);
        if (change == true) {
          setChange(false);
        } else {
          setChange(true);
